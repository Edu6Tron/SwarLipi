...
function BackupSheet({ library, onDismiss, visible }: { library: LibraryState; onDismiss: () => void; visible: boolean }) {
  ...
  async function uploadToGitHub() {
    ...
    try {
      ...
      showFeedback("success", "Encrypted library synced", `A protected snapshot is now stored in ${session.repository ?? "your private repository"}. To update another device, open Private backup there, use the same passphrase, then choose Fetch encrypted backup to review.`);
      feedback.confirm();
    } catch (error) {
  ...
  async function prepareRestore() {
    ...
    try {
      ...
      showFeedback("success", "Restore is ready to review", `The encrypted copy contains ${restored.texts.length} texts. Your current library is still unchanged. Use Replace this device with restored library only when you are ready to update it.`);
      feedback.confirm();
    } catch (error) {
  ...
  async function applyRestore() {
    ...
    setWorking(false);
    setActiveAction(null);
      showFeedback("success", "Library restored on this device", "This device now matches the decrypted GitHub copy and is saved for offline reading.");
    feedback.confirm();
  }
        ...
        <ScrollView contentContainerStyle={styles.backupScroll} keyboardShouldPersistTaps="handled">
          ...
          {!session ? (
            ...
            <View style={styles.repositorySetup}>
              ...
              {session.repository ? <><View style={styles.crossDeviceRestoreCard}><MaterialIcons name="sync" size={21} color="#9DE4D7" /><View style={styles.crossDeviceRestoreCopy}><Text style={styles.crossDeviceRestoreTitle}>Bring the browser backup onto this device</Text><Text style={styles.crossDeviceRestoreText}>Enter the same passphrase above, tap Fetch backup, review the count, then tap Replace. Nothing updates automatically, so new phone writing is not erased by mistake.</Text></View></View><Text style={styles.inputLabel}>STEP 2 · BACKUP OR RESTORE</Text><Text style={styles.repositoryGuide}>Choose the first action to back up this device. Choose the second action to update this device from a browser or other device backup.</Text><Pressable disabled={working} onPress={() => void uploadToGitHub()} style={({ pressed }) => [styles.backupUploadButton, (working || pressed) && styles.saveButtonPressed]}>{activeAction === "sync" ? <ActivityIndicator size="small" color="#271116" /> : <MaterialIcons name="cloud-upload" size={19} color="#271116" />}<Text style={styles.backupPrimaryButtonText}>{activeAction === "sync" ? "Syncing encrypted library…" : "Back up this device to GitHub"}</Text></Pressable><Pressable disabled={working} onPress={() => void prepareRestore()} style={({ pressed }) => [styles.restoreButton, (working || pressed) && styles.saveButtonPressed]}>{activeAction === "restore" ? <ActivityIndicator size="small" color="#D6C5DD" /> : <MaterialIcons name="cloud-download" size={19} color="#D6C5DD" />}<Text style={styles.restoreButtonText}>{activeAction === "restore" ? "Fetching encrypted backup…" : "Update this device from backup"}</Text></Pressable>{restoreReady ? <Pressable disabled={working} onPress={() => void applyRestore()} style={({ pressed }) => [styles.replaceLibraryButton, (pressed || working) && styles.saveButtonPressed]}>{activeAction === "apply-restore" ? <ActivityIndicator size="small" color="#2B1419" /> : <MaterialIcons name="warning-amber" size={19} color="#2B1419" />}<Text style={styles.backupPrimaryButtonText}>{activeAction === "apply-restore" ? "Replacing library…" : "Replace this device with restored library"}</Text></Pressable> : null}</> : null}
...
const styles = StyleSheet.create({
  ...
  crossDeviceRestoreCard: { flexDirection: "row", alignItems: "flex-start", gap: 10, marginTop: 16, marginBottom: 16, padding: 14, borderRadius: 17, backgroundColor: "#1A2928", borderWidth: 1, borderColor: "#385954" },
  crossDeviceRestoreCopy: { flex: 1 },
  crossDeviceRestoreTitle: { color: "#C8F1E8", fontSize: 12, fontWeight: "800", lineHeight: 17 },
  crossDeviceRestoreText: { color: "#B2C8C2", fontSize: 11, lineHeight: 16, marginTop: 4 },
