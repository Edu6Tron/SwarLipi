...
function getClientId() {
  return process.env.EXPO_PUBLIC_GITHUB_APP_CLIENT_ID?.trim() || DEFAULT_GITHUB_APP_CLIENT_ID;
}

function getAppSlug() {
  return process.env.EXPO_PUBLIC_GITHUB_APP_SLUG?.trim() || DEFAULT_GITHUB_APP_SLUG;
}
...
async function writePersistedSession(session: GitHubSession) {
  ...
  if (Platform.OS === "web") {
    globalThis.localStorage?.setItem(GITHUB_ACCOUNT_KEY, serialized);
    return;
  }
...
async function clearPersistedSession() {
  if (Platform.OS === "web") {
    globalThis.localStorage?.removeItem(GITHUB_ACCOUNT_KEY);
    return;
  }
...
export async function loadGitHubSession(): Promise<GitHubSession | null> {
  try {
    const raw = Platform.OS === "web"
      ? globalThis.localStorage?.getItem(GITHUB_ACCOUNT_KEY) ?? null
      : await SecureStore.getItemAsync(GITHUB_ACCOUNT_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as GitHubSession;
    if (!parsed.accessToken || !parsed.profile?.login) return null;
...
export async function forgetGitHubSession() {
  await clearPersistedSession();
}
...
async function githubRequest<T>(path: string, accessToken: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    if (response.status === 401) throw new Error("Your GitHub session has expired. Sign in again to continue.");
    throw new Error(payload.message ?? "GitHub did not accept this request.");
  }
  return payload;
}
...
export async function beginGitHubDeviceLogin(): Promise<GitHubDeviceCode> {
  ...
  if (!clientId) throw new Error("GitHub sign-in is being prepared. Add the GitHub App client ID to activate it.");
  ...
  if (!response.ok || !payload.device_code || !payload.user_code || !payload.verification_uri) {
    throw new Error(payload.error_description ?? "GitHub could not start the sign-in request.");
  }
  return {
...
export async function openGitHubDeviceLogin(verificationUri: string) {
  if (Platform.OS === "web") {
    globalThis.open(verificationUri, "_blank", "noopener,noreferrer");
    return;
  }
...
export async function finishGitHubDeviceLogin(deviceCode: string, keepSignedIn: boolean): Promise<GitHubSession> {
  ...
  if (!response.ok || !token.access_token) {
    if (token.error === "authorization_pending") throw new Error("GitHub confirmation is still pending. Finish it in the browser, then check again.");
    throw new Error(token.error_description ?? "GitHub sign-in could not be completed.");
  }
  ...
  const session: GitHubSession = {
    ...
    repository: null,
    profile: { login: profile.login, avatarUrl: profile.avatar_url ?? null },
    keepSignedIn,
  };
...
export async function selectPrivateGitHubRepository(session: GitHubSession, repository: string): Promise<GitHubSession> {
  ...
  const next = { ...session, repository: normalized };
  await persistGitHubSession(next);
  return next;
}
...
export async function uploadEncryptedLibrary(session: GitHubSession, envelope: EncryptedBackupEnvelope) {
  ...
  try {
    const existing = await githubRequest<{ sha?: string }>(filePath, session.accessToken);
    sha = existing.sha;
  } catch (error) {
    if (!(error instanceof Error) || !error.message.includes("Not Found")) throw error;
  }
  await githubRequest(filePath, session.accessToken, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      message: `SwarLipi encrypted sync ${envelope.createdAt}`,
      content: encodeBase64(JSON.stringify(envelope)),
      ...(sha ? { sha } : {}),
    }),
  });
}
...
export async function fetchEncryptedLibrary(session: GitHubSession): Promise<EncryptedBackupEnvelope> {
  ...
  try {
    return JSON.parse(decodeBase64(result.content)) as EncryptedBackupEnvelope;
  } catch {
    throw new Error("The encrypted backup in this repository is not valid SwarLipi data.");
  }
}
...
export async function openGitHubAppInstallation() {
  const url = githubAppInstallUrl();
  if (!url) throw new Error("GitHub App installation is being prepared. Add the GitHub App slug to activate it.");
  await openGitHubDeviceLogin(url);
}
